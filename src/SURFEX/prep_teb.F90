...
!     #########
SUBROUTINE PREP_TEB (DTCO, UG, U, USS, GCP, TOP, BOP, NB, TG, SB, NT, GDM, GRM, NPAR_VEG_IRR_USE, &
...
DO JP=1,TOP%NTEB_PATCH
  ...
  !
  CALL PREP_HOR_TEB_FIELD(NB%AL(JP), BOP, DTCO, U, GCP, TG, NT%AL(JP), TOP, &
                         HPROGRAM,'TI_ROAD',HATMFILE,HATMFILETYPE,HPGDFILE,HPGDFILETYPE, JP,NPAR_VEG_IRR_USE,YDCTL)
  !
  ...
  IF (TOP%LGARDEN)    CALL PREP_TEB_GARDEN(DTCO, UG, U, USS,  GCP, TG, TOP, &
                                           GDM%O, GDM%S, GDM%K, GDM%P, GDM%NPE%AL(JP), NPAR_VEG_IRR_USE,  &
